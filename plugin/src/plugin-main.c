/*
 * obs-spark-libre — plugin entry point.
 *
 * Responsibilities of this thin native layer:
 *   1. Identify itself to OBS (OBS_DECLARE_MODULE).
 *   2. On load, register a Qt dock that embeds the Spark Libre web UI.
 ...
 *
 * The dock registration itself is implemented in C++ (spark-dock.cpp)
...
#ifndef PLUGIN_NAME
#  define PLUGIN_NAME "obs-spark-libre"
#endif
...
{
    return "Embeds the Spark Libre web UI as a native OBS dock and forwards "
           "user actions to the external backend on 127.0.0.1:8765.";
}

/* Implemented in spark-dock.cpp + spark-actions.cpp + spark-websocket-bootstrap.cpp */
extern bool spark_dock_register(void);
extern void spark_dock_unregister(void);
extern void spark_actions_install(void);
extern void spark_actions_uninstall(void);
extern void spark_websocket_bootstrap(void);
...
void obs_module_unload(void)
{
    ...
    spark_actions_uninstall();
    spark_dock_unregister();
}
